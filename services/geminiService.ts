...
export async function* streamDefinition(
  ...
  const prompt = `Stellen Sie eine prägnante, ein-Absatz-Enzyklopädie-artige Definition für den Begriff bereit: "${topic}". Seien Sie informativ und neutral. Verwenden Sie kein Markdown, keine Titel oder spezielle Formatierungen. Antworten Sie nur mit dem Text der Definition selbst.`;
...
export async function getRandomWord(): Promise<string> {
  ...
  const prompt = `Generieren Sie ein einzelnes, zufälliges, interessantes deutsches Wort oder ein Zwei-Wort-Konzept. Es kann ein Substantiv, Verb, Adjektiv oder ein Eigenname sein. Antworten Sie nur mit dem Wort oder Konzept selbst, ohne zusätzlichen Text, Satzzeichen oder Formatierung.`;
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : 'Ein unbekannter Fehler ist aufgetreten.';
    throw new Error(`Zufälliges Wort konnte nicht abgerufen werden: ${errorMessage}`);
  }
}
...
export async function generateAsciiArt(topic: string): Promise<AsciiArtData> {
  ...
  - Palette: │─┌┐└┘├┤┬┴┼►◄▲▼○●◐◑░▒▓█▀▄■□▪▫★☆♦♠♣♥⟨⟩/\\_|
  - Die Form spiegelt das Konzept wider - lassen Sie die visuelle Form die Essenz des Wortes verkörpern
  - Beispiele: 
    * "Explosion" → strahlenförmige Linien vom Zentrum aus
    * "Hierarchie" → Pyramidenstruktur
    * "Fluss" → gekrümmte Richtungslinien
  - Als einzelne Zeichenkette mit \n für Zeilenumbrüche zurückgeben`;


  const keysDescription = `einen Schlüssel: "art"`;
  const promptBody = artPromptPart;

  const prompt = `Erstellen Sie für "${topic}" ein JSON-Objekt mit ${keysDescription}.
${promptBody}

Geben Sie NUR das rohe JSON-Objekt zurück, keinen zusätzlichen Text. Die Antwort muss mit "{" beginnen und mit "}" enden und nur die Eigenschaft "art" enthalten.`;
